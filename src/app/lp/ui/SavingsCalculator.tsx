              ...
              <input
                ...
                */
                onChange={(e) => {
                  ...
                  setAttempted(false);
                  // ⚠️ וגם `blurred` עצמו: אחרי יציאה ראשונה מהשדה הוא נשאר
                  // דלוק לתמיד, וחזרה לשדה כדי להקליד `1,200` הכריזה שוב
                  // על `1,` `1,2` `1,20` — בדיוק הרגרסיה שההערה למעלה
                  // מתארת כמתוקנת. יציאה מהשדה מדליקה אותו מחדש.
                  setBlurred(false);
                }}
                ...
                */
                ...
                /*
                  ...
                  `₪ 5,000.00` הוא הארוך ביותר שהמחשבון מטפל בו — אבל
                  העתקה מחשבונית או מדף עברי נושאת גם סימני כיווניות
                  (U+200E/200F), וה-`maxLength` סופר אותם **לפני** שהם
                  מנוקים ב-`parseSpendRaw`. `‏₪ 5,000.00‏` הוא 12 תווים,
                  נחתך ל-`₪ 5,000.` ונפסל — שוב האשמת המשתמש בקלט תקין.
                  התקרה כאן אינה השומר; `parseSpendRaw` הוא.
                */
                maxLength={16}
