...
const RISE_IN_TEXT =
  ...
  // חודשיים.
  //
  // ⚠️ `חודש(?:ים|יים|ם)? ר[אוש]{2,3}נים` ולא `חודשי?ים ראשונים`: HOT
  // "1000/100 ללא VOD" כותבת "3 חודשים ראושנים 99" ו-"כולל VOD" כותבת
  // "3 חודשם ראשונים 129" — שגיאות הקלדה של מי שהזין את הקטלוג, ושתיהן
  // חמקו. היום ה-`price` שלהן (169/199) הוא כבר המחיר הרגיל ולכן
  // הכותרת לא נפגעה, אבל הן עברו במקרה: ברענון שישמור 99 ב-`price`
  // הכותרת נבנית על מחיר של שלושה חודשים. הצהרה עם שגיאת כתיב היא
  // עדיין הצהרה.
