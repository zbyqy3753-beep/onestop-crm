 ...
 *
 * ⚠️⚠️ **התדירות שם היא `* * * * *` — כל דקה — וזה לא נוי.**
 *
 * מילוי התור קורה רק בתקתוק (`enqueueDueFollowUps` נקרא מתוך
 * `pull`), ולכן רזולוציית השעון היא גם הדיוק של התזכורת. בתקתוק של
 * כל חמש דקות, חזרה שנקבעה ל-10:31 לא נכנסה לתור ב-10:20 (עדיין לא
 * בשלה) ונכנסה רק ב-10:25 — כלומר ההקדמה של עשר דקות הצטמצמה לשש,
 * ובנוסח הישן ההודעה אף הכריזה על עצמה "באיחור". חזרה בשעה עגולה
 * נפלה על תקתוק ויצאה נכון, ולכן זה נראה אקראי.
 *
 * הרצה כפולה בטוחה מעצם הבנייה — התביעה היא `updateMany` מותנה על
 * `status: "queued"` — ולכן קיצור המרווח אינו מסתכן בכפילות.
 *
