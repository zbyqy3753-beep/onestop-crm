...
/**
 * למי לשייך את הליד הנכנס — הנציג הפעיל עם הכי מעט לידים פתוחים.
 *
 * "הכי פחות עמוס" ולא סבב מחזורי, כי סבב דורש לזכור מי היה אחרון;
 * העומס הנוכחי כבר יושב ב-DB ומתקן את עצמו — נציג שסוגר לידים מהר
 * מקבל את הבא בתור. שוויון נשבר לפי סדר השמות (`listActive` ממיין
 * בא״ב), כך שהתוצאה דטרמיניסטית.
 *
 * `undefined` = הליד נשאר ללא שיוך: או שהחלוקה כבויה, או שאין אף
 * נציג פעיל. במקרה כזה הליד נוחת במאגר המשותף, כמו קודם.
 */
async function nextAssignee(): Promise<string | undefined> {
  if (process.env.LEADS_API_AUTO_ASSIGN === "off") return undefined;
