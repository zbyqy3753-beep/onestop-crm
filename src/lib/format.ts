...
export function normalizeIsraeliPhone(raw: string): string | null {
  ...
  //
  // ⚠️ `9720?` ולא `972`: מי שמקליד ידנית `+972 050-1234567` משאיר את
  // האפס המקומי אחרי הקידומת הבינלאומית, והתוצאה הייתה `00501234567` —
  // 11 ספרות שנפסלות, כלומר מספר ישראלי תקין לגמרי שנדחה. `972` שאחריו
  // אפס הוא תמיד כפל-קידומת ולא מספר לגיטימי.
