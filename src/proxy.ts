...
const PUBLIC_PREFIXES = [
  "/form",
  /*
   * ⚠️ דף הנחיתה חייב להיות פתוח **לגמרי**, ולא רק פטור ממפתח הגישה:
   * מי שמגיע אליו הוא לקוח שקיבל את הקישור בוואטסאפ ואין לו שום עוגייה
   * ושום חשבון. כל דבר שאינו 200 נראה לו כמו קישור שבור.
   *
   * מה שמגן על המערכת הוא שהנתיב לא מוביל לשום מקום: הדף אינו מקושר
   * ל-`(app)`, וה-Server Action שלו כותב ליד ואינו קורא כלום.
   */
  "/lp",
  /*
   * ⚠️ הלוגואים של החברות, שכרטיסי החבילות בדף הנחיתה מציגים. חייבים
   * להיות פתוחים בנפרד: הדפדפן מושך אותם כמשאב ולא כניווט, והשער
   * החזיר להם את דף ההתחברות כ-HTML — מה שגרם ל-`next/image` להחזיר
   * 400, וכל 106 הכרטיסים הוצגו בלי לוגו ובלי שום שגיאה שמסבירה למה.
   */
  "/providers",
  /*
   * ⚠️ תצלום ההירו והאריה של דף הנחיתה — אותה מלכודת כמו `/providers`
   * בדיוק מעל, ולכן אותה שורה. `next/image` מושך את הקובץ דרך שרת
   * ההמרה, מקבל מהשער את דף ההתחברות כ-HTML, ומחזיר 400 עם "הקובץ
   * המבוקש אינו תמונה תקינה" — הודעה שאינה רומזת שהאשם הוא הרשאה.
   */
  "/brand",
  "/api",
  "/_next",
   ...
   */
  "/set-password",
  // ⚠️ מאותה סיבה בדיוק: מי שמגיע לכאן הוא מי שאין לו דרך להיכנס.
  "/forgot-password",
];
