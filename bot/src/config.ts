...
/**
 * כמה לחכות לאישור קבלה של וואטסאפ לפני שמכריזים כישלון.
 *
 * ⚠️ קיים כי `sendMessage` שחוזר בהצלחה אומר רק "נמסר לספרייה", ולא
 * "וואטסאפ קיבלו". אחרי צימוד טרי או סשן חלש ההודעה נעלמת בלי שום
 * שגיאה — והמערכת דיווחה "נשלח" על משהו שהלקוח לא קיבל. זה קרה בפועל.
 */
