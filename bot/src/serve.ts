...
/**
 * המתנה אחרי שמופע אחר תפס את החיבור (440).
 *
 * ארוכה בכוונה — ראה ההערה ב-`onClosed`. שתי שניות הופכות התנגשות
 * בין שני בוטים ללולאה אינסופית שאף אחד לא מנצח בה.
 */
const REPLACED_RETRY_MS = 60_000;
    ...
    try {
      wa = await connect({
        ...
        onClosed: (reason) => {
          ...
          /*
           * ⚠️ 440 = connectionReplaced — מופע אחר תפס את החיבור.
           *
           * זה **לא** ניתוק רגיל, וחיבור מחדש מיידי הוא בדיוק הדבר הלא
           * נכון: השני יעוף, יתחבר בחזרה, יעיף אותנו, וחוזר חלילה. זה
           * נצפה בשטח כלולאה של שלוש שניות שרצה בלי סוף ומייצרת לוג
           * שנראה כמו תקלת רשת — בזמן שהסיבה היא פשוט שני בוטים על
           * אותה תיקיית `auth`.
           *
           * דקה שלמה במקום שתי שניות: אם השני נסגר בינתיים נתאושש לבד,
           * ואם לא — הלוג אומר את הסיבה במפורש במקום להסתיר אותה ברעש.
           */
          if (reason === DisconnectReason.connectionReplaced) {
            log("✗ מופע אחר של הבוט תפס את החיבור (440).");
            log("  שני בוטים רצים על אותה תיקיית auth — סגור את השני.");
            log(`  ניסיון נוסף בעוד ${REPLACED_RETRY_MS / 1000}ש׳.`);
            setTimeout(() => void open(), REPLACED_RETRY_MS);
            return;
          }
